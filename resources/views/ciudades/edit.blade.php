      ...
      <div class="card my-4">
          <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
              <div class="bg-gradient-info shadow-info border-radius-lg pt-4 pb-3">
                  <h6 class="text-white text-capitalize ps-3">Acutalizar CIudad/Provincia: {{ $provincia->nombre }}</h6>
              </div>
          </div>
          <div class="card-body">
              @if ($errors->any())
                  ...
                  <div class="form-control">
                      <div class="input-group input-group-static mb-4">
                          ...
                          <input type="text" name="nombre" class="form-control"
                              value="{{ old('nombre', $provincia->nombre) }}" id="nombre" required>
                      </div>
                  </div>
                  <div class="form-control">
                    <div class="input-group input-group-static mb-4">
                        ...
                        <select class="form-control" id="pais_id" name="pais_id" required>
                          @foreach($paises as $pais)
                              <option value="{{ $pais->id }}" {{ $provincia->pais_id == $pais->id ? 'selected' : '' }} >{{ $pais->nombre }}</option>
                          @endforeach
                      </select>
                    </div>
                </div>
